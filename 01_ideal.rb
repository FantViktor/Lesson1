=begin Идеальный вес. Программа запрашивает у пользователя имя и рост и выводит
идеальный вес по формуле (<рост> - 110) * 1.15, после чего выводит
результат пользователю на экран с обращением по имени. Если идеальный
вес получается отрицательным, то выводится строка "Ваш вес уже оптимальный"
ПРИМЕЧАНИЕ Идеальный вес по формуле Брока
=end

puts "Введите своё имя"
name =gets.chomp
puts"Введите свой рост"
height=gets.chomp.to_i

num = (height.to_i - 110)*1.15
if num < 100
  puts "#{name}, твой идеальный вес #{num}"
else num > 0
  puts "Ваш вес уже оптимальный"
end